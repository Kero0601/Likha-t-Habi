...
const CheckoutPage = () => {
  // 1. Get 'products' from context to check real-time stock
  ...
  // --- NEW: VALIDATE STOCK BEFORE PROCEEDING ---
  const validateStock = () => {
    for (const item of itemsToBuy) {
        // Find the latest product data from Context (Real-time)
        ...
        if (freshProduct) {
            ...
            // 1. Check if Sold Out completely
            if (currentStock <= 0) {
                toast.error(`Sorry, '${item.name}' is now SOLD OUT!`, { toastId: 'checkout-soldout' });
                return false;
            }
            // 2. Check if ordering more than available
  ...
  const handlePlaceOrderClick = () => {
    ...
    // Check Stock Before Opening Payment Modal
  ...
  const processOrder = async () => {
    ...
    // Final Stock Check (Just in case stock changed while typing Ref #)
    if (!validateStock()) return;
    ...
    if (success) { 
        ...
        // FIX: Removed toast.error() here because placeOrder() in Context already shows the specific error.
        // This prevents the double notification.
    }
  };
        ...
        <div className="lh-checkout-container">
          ...
          <div className="lh-grid">
            <div className="lh-col-left">
              ...
              <div className="lh-card skeleton-card"><div className="skeleton-title-bar"></div>{[1, 2].map((i) => (<div key={i} className="skeleton-item-row"><div className="skeleton-img"></div><div className="skeleton-text-group"><div className="skeleton-line lg"></div><div className="skeleton-line sm"></div></div></div>))}</div>
            </div>
